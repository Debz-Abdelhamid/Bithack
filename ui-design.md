...
| `/tickets` | **drag‑and‑drop Kanban board** + filter chips + stats footer (MTTR, SLA compliance, pending approval, critical flares) | **Delivered, Phase 7 (2026-07-08)** — `MaintenanceBoard` custom Filament Page, native HTML5 drag-and-drop (Alpine `x-on:dragstart/dragover/drop`), 5 columns matching `TicketStatus::boardColumns()`. `mokhosh/filament-kanban` evaluated first per `Phases.md`'s instruction — it requires Filament `^3.0` and has no official v4 support (only an unofficial third-party fork claims v4 compatibility); given the legacy board itself turned out to be plain native drag-and-drop with no client library at all (confirmed by reading `TicketsBoardView.tsx`, not guessed), built custom rather than depend on an unofficial fork. Filter chips/stats footer from the legacy board **not** ported — flagged as a nice-to-have, not requested. |
| `/tickets/[id]` | ticket detail: activity timeline, comment + photo modals, QR-scan description block | **Delivered, Phase 7** — `MaintenanceTicketResource`'s View page: `InterventionsRelationManager` (technician/schedule/cost/report) + "Advance status"/"Cancel ticket" actions, both routed through `TicketWorkflowService` so a manual change and a Kanban drag can never enforce different rules. Comment/photo modals from the legacy detail view **not** ported — `activitylog` already captures a status-change trail; a dedicated comment/photo thread is a nice-to-have, not requested. |
| `/reservations` | **custom weekly timetable grid** (`calendar-grid`), filters Faculté→Département→Spécialité, week nav, slot statuses confirmed/pending/changed | Split across three surfaces (Phase 5 addendum, 2026-07-06): `RoomReservationResource` (N2/A3 table — confirm/reject ad-hoc requests, list/edit timetable rows); `TimetableBuilder` custom page — **the visual grid, ported as-is** (6 fixed periods × Sat–Thu columns from `data.ts`'s exact seed values, §9.5), the primary way N2 (own faculty's departments) / A3 fills a department's timetable for a picked Academic Term, with a side "Add to timetable" panel matching the legacy `ReservationsView.tsx`'s Assignment Builder panel; `RequestReservation` — Enseignant's ad-hoc/one-off booking form. **`timetable` slots entered directly `confirmed`** — the faculty's emploi du temps; **`request` slots** ad-hoc, `pending` until N2/A3 confirms. `tout_utilisateur`/everyone sees the read-only `ReservationAvailability` grid (room-based, not department-based). **Design-fidelity pass (2026-07-06, owner-requested)**: `TimetableBuilder` now ports the legacy 3-column layout — a left "Academic Structure" sidebar (Faculty→Department read-only tree, click-to-select, replaces a plain department dropdown; specialities/groups/class-levels from the legacy sidebar are deliberately **not** reproduced — out of scope, no such entities exist in R13, see `Schema.md` §6 open question #4) — plus richer card anatomy (Heroicon lines for level·student_group / teacher / room·building, a status badge, hover-reveal cancel) and a status legend footer. The legend intentionally shows only **Confirmed** (the grid's `getGridSlots()` query only ever returns `status=confirmed` rows by construction — Pending/Changed from the legacy's 3-state legend would be permanently-dead UI here, since ad-hoc/pending flow lives on `RequestReservation` instead). |
| `/report/[code]` | **QR landing page, mobile-first**: auth-gated, shows asset summary + status badge, one-textarea report form, duplicate-ticket guard ("Already Reported" if active ticket), success screen with ticket ref | **Delivered, Phase 6 (2026-07-07)** — `GET /report/{token}` (`QrLookupController`, unchanged from Phase 3) stays public/unauthenticated — a deliberate divergence from the legacy's page-level auth gate, preserving Phase 3's already-shipped public contract; only `POST /report/{token}` (`AnomalyReportController`) requires a session. A guest sees the same read-only card plus a "log in to report" link instead of the form. Matched the legacy exactly on priority: always `urgent`/48h for a QR-scan report (`ReportView.tsx` never offered a category picker), duplicate-ticket guard via `MaintenanceTicket::hasActiveTicketFor()`, success state shows the `TCK-YYYY-NNNNN` reference. Plain Blade route **outside** the admin panel, its own Redis limiter (`anomaly-report`, per-user + per-token, `Security.md` §5). |
