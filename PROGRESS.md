...
**Last update:** 2026-07-06 — Phases 0–5 complete · next: **Phase 6 (Anomaly reporting → auto ticket)** — waiting for owner go
...
| 5 — Room reservations | ✅ | `room_reservations` (§2.7) with a **`source` split** (2026-07-06 reversal, see decision log): `timetable` (N2/A3-authored, created directly `confirmed`, teacher picked from a dropdown) vs `request` (Enseignant ad-hoc, starts `pending`). Pg **EXCLUDE USING gist** (btree_gist, partial `WHERE status='confirmed'`) verified rejecting a raw overlapping insert + `RoomReservationObserver` app-level guard on every driver (defense in depth, matches Phase 4's CHECK-constraint pattern). Recurrence = generated weekly rows sharing `recurring_group_id` (documented addition beyond §2.7); whole-series pre-validated, nothing persisted on any conflict. **`ReservationApprovalService`**: confirm (re-checks overlap, auto-rejects other overlapping pending requests — open question #5 default) / reject (optional reason) / notifyApprover (room's-faculty N2, or A3 for shared rooms). Two custom Filament pages alongside the N2/A3 admin resource: `RequestReservation` (Enseignant-only ad-hoc form + own-requests cancel, campus-wide catalog, per-user hourly rate limit) and `ReservationAvailability` (read-only weekly grid, every role). Queued `SendReservationNotification` job (request rows only, Security.md §7 payload-minimized). **Bug found & fixed during testing**: business logic dereferencing `$reservation->local->building` silently returned null when the acting user's own faculty differed from the room's (Local/Building's own FacultyScope is keyed to *whoever is authenticated*, not the reservation's owner) — fixed with explicit `withoutGlobalScope` reads in the service and in `myRequests()`, both eager-load levels (local *and* nested building). |

**Tests:** 118 green (sqlite + pgsql in CI). **English-first UI** (owner decision; `lang/en` primary, `lang/fr` maintained). **Timezone:** Africa/Algiers.
...
5. Phase 5 defaults — **implemented, not yet confirmed with the university**: `timetable` recurrence capped at `PATRIMO_RESERVATION_MAX_RECURRENCE_MONTHS` (default 4 months, `config/patrimo.php`); confirming a pending `request` auto-rejects other overlapping pending requests for the same room/time (notified as "rejected — room taken").
